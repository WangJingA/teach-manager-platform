<template>
	<div>
        ...
        <el-select v-model="query.address" placeholder="课程属性" class="handle-select mr10">
          <el-option key="1" label="自建课程" value="自建课程"></el-option>
          <el-option key="2" label="分配课程" value="分配课程"></el-option>
        </el-select>
        ...
        <el-link href="/template.xlsx" target="_blank" class="search-button">
          <el-button class="mr10 search-button "  type="success" >下载模板</el-button>
			...
			<el-table :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header">
				<el-table-column prop="id" label="课程口令" width="220" align="center"></el-table-column>
				<el-table-column prop="name" label="课程所在学院"></el-table-column>
				<el-table-column label="课程所属专业">
					<template #default="scope">￥{{ scope.row.money }}</template>
				</el-table-column>
        <el-table-column prop="name" label="课程所属班级"></el-table-column>
				<el-table-column label="课程封面(查看大图)" align="center">
        ...
        <el-table-column label="自建课？/分配课？" align="center">
          <template #default="scope">
            <el-tag
                :type="scope.row.state === '成功' ? 'success' : scope.row.state === '失败' ? 'danger' : ''"
            >
